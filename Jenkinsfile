pipeline {
    ...
    environment {
        ...
        PORTAINER_DEV_WEBHOOK = credentials('PORTAINER_WEBHOOK_DEV_IPERF')
        PORTAINER_PRD_WEBHOOK = credentials('PORTAINER_WEBHOOK_PRD_IPERF')
    }
    ...
    stages {
        ...
        stage('deploy dev code') {
            ...
            steps {
                ...
                sh 'ansible-playbook ${ANSIBLE_REPO}/deploy/docker/deploy_docker_compose_dev.yml --extra-vars repo="iperf"'
            }
        }
        ...
        stage('deploy prd code') {
            ...
            steps {
                ...
                sh 'ansible-playbook ${ANSIBLE_REPO}/deploy/docker/deploy_docker_compose_prd.yml --extra-vars repo="iperf"'
            }
        }
        // trigger portainer redeploy
        // separated out so this only gets run if the ansible playbook doesn't fail
